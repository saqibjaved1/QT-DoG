...
class INQController(Controller):
    ...
    def step_pre_training(self, epoch, optimizer=None, logbook=None):
        """Call this each epoch before training loop."""
        if epoch in self.schedule.keys():
            self.fraction = self.schedule[epoch]
        else:
            if logbook is not None and logbook.is_master:
                logbook.writer.add_scalar('INQ/fraction', self.fraction, global_step=epoch)
...
class INQNodeController:
    ...
    def inq_step(self, fraction):
        ...
        if self.quant_strategy == "magnitude-SRQ" or self.quant_strategy == "RPR":
        ...
        elif self.quant_strategy == "filter_inq" or self.quant_strategy == "magnitude" or self.quant_strategy == "low_magnitude" or self.quant_strategy == "random" or self.quant_strategy =="structural":
            ...
            if self.quant_strategy == "magnitude":
            ...
            elif self.quant_strategy == "low_magnitude":
                idx_full_prec = torch.nonzero(torch.isnan(self.weight_frozen.flatten()))
                if idx_full_prec.numel() > 0:
                    idx_full_prec = idx_full_prec[:, 0]
                _, idx = self.weight.flatten()[idx_full_prec].abs().sort(descending=False)
                idx_new_quant = idx_full_prec[idx[:new_count-old_count]]

            elif self.quant_strategy == "structural":
                idx_full_prec = torch.nonzero(torch.isnan(self.weight_frozen[:, ]).any(1).sum((1, 2)))
                _, idx = self.weight.data[idx_full_prec].sum((1, 3)).sum((2, 1)).abs().sort(descending=False)
                n_weights = self.weight_frozen.shape[0]
                old_count = n_weights - idx_full_prec.shape[0]
                new_count = int(self.fraction * n_weights)
                self.weight_frozen[idx_full_prec[idx[:new_count-old_count]]] = self.inq_quantize(self.weight.data[idx_full_prec[idx[:new_count-old_count]]], quant_levels)

            elif self.quant_strategy == "filter_inq":
                frac = int(0.9*self.weight_frozen.shape[0])
                idx_full_prec = torch.nonzero(torch.isnan(self.weight_frozen[:frac].flatten()))
                if idx_full_prec.numel() > 0:
                    idx_full_prec = idx_full_prec[:, 0]
                _, idx = self.weight.flatten()[idx_full_prec].abs().sort(descending=True)
                n_weights = self.weight_frozen[:frac].numel()
                old_count = n_weights - torch.isnan(self.weight_frozen[:frac].data).sum(dtype=torch.long).item()
                new_count = int(self.fraction * n_weights)
                idx_new_quant = idx_full_prec[idx[:new_count - old_count]]
                ...
                assert False
            if self.quant_strategy != "structural":
